...
class Comaparer{
    public:
    // ports definition
    using input_ports=tuple<typename Comaparer_defs::gameActionIn1, typename Comaparer_defs::gameActionIn2, typename Comaparer_defs::playGameStartIn>;
    ...
    void internal_transition() {
        if (state.active == true && received1 == true && received2 == true) {
    ...
    void external_transition(TIME e, typename make_message_bags<input_ports>::type mbs) { 
        if (get_messages<typename Comaparer_defs::playGameStartIn>(mbs).size() > 1) {
        ...
        if (player1[0] != -1) {
            ...
            state.receieved1 = true;
        }
        if (player2[0] != -1) {
            state.playerResult2 = player2[0];
            state.receieved2 = true;
        }
    }
    ...
    typename make_message_bags<output_ports>::type output() const {
        ...
        else if (state.active == true && received1 == false && received2 == false) {
